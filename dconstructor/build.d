...
string declareInterceptors (TInterceptor...) () {
    string ret = ``;
    foreach (i, TI; TInterceptor) {
        string num = ToString!(i);
        ret ~= `typeof(TInterceptor[` ~ num ~ `]) intercept_` ~ num ~ `;`;
    }
    return ret;
}

string buildInterceptors (TInterceptor...) () {
    string ret = `this () {`;
    foreach (i, TI; TInterceptor) {
        ret ~= `intercept_` ~ ToString!(i) ~ 
            ` = new typeof(TInterceptor[` ~ ToString!(i) ~ `])();`;
    }
    ret ~= `}`;
    return ret;
}

string useInterceptors (TInterceptor, int i = 0) () {
    if (i < TInterceptor.length) {
        return `intercept_` ~ ToString!(i) ~
            `.intercept(obj);` ~
            declareInterceptors!(TInterceptor, i + 1)();
    } else {
        return ``;
    }
}
...
/**
  ...
  * The template arguments TInterceptor... are interceptors. They hook into
  * the build process and can modify an object or simply record its occurrence
  * as it is being built. An interceptor must define a template method
  * void intercept(T)(T obj). This template should not barf on structs, arrays,
  * associative arrays, primitives, or objects, ideally.
  * 
  * The recommended way of using a Builder with interceptors is to create a
  * module for it. Let's say I have an EventInterceptor in events.intercept.
  * In my project, I define a module:
  * ---
  * module myproject.build;
  * import events.intercept;
  * import dconstructor.build;
  * public Builder!(EventInterceptor) builder;
  * static this () {
  *     builder = new Builder!(EventInterceptor)();
  * }
  * ---
  *
...
class Builder (TInterceptor...) {
    pragma(msg, declareInterceptors!(TInterceptor)());
    mixin(declareInterceptors!(TInterceptor)());
    pragma(msg, buildInterceptors!(TInterceptor)());
    mixin(buildInterceptors!(TInterceptor)());
    /**
    ...
    T get(T)() {
        ...
        auto b = get_or_add!(T)();
        T obj = b.build(this);
        pragma(msg, useInterceptors!(TInterceptor)());
        mixin(useInterceptors!(TInterceptor)());
...
public Builder!(InterceptOne) builder;

static this () {
    builder = new Builder!(InterceptOne)();
}
